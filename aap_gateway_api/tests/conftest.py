...
def ensure_jwt_keys(request):
    ...
    if not connection.in_atomic_block:
        ...
        # Explicit commit so subprocesses (which open their own DB connections)
        # can see the keys.  Non-transactional tests run inside an atomic block
        # where the write is already visible within the same connection.
        transaction.commit()
        connection.close()
        time.sleep(0.1)
